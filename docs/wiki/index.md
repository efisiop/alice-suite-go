...
| Reader and Consultant paper theme | done | user + agent | `internal/static/css/paper-theme.css`, `internal/templates/base.html`, `docs/APP_UPGRADES.md` (warm shared surfaces, deeper top and Reader side framing) |
| Reader printed-paper text area | done | user + agent | `internal/templates/reader/interaction.html` (warm ivory paper, serif ink, quiet dotted lookup marks, responsive margins; no new assets), `docs/APP_UPGRADES.md`, `docs/READER_AUTORESEARCH.md` |
...
| Reader registration data-use notice | in-progress | user + agent | `internal/templates/reader/register.html`, `internal/handlers/reader_routes_test.go`, `docs/EVOLUTION_CONTROL.md` (account identity and consultant-support purpose disclosed; registration is not promotional consent; a complete Privacy Notice and actual Info Center opt-in remain to be added) |
...
| Consultant UX upgrades         | in-progress | user + agent | `docs/UPGRADE_ROADMAP.md`, `docs/AGENT_LOOP_INVENTORY.md`, `cmd/server/main.go`, `internal/database/consultant_assignments.go`, `internal/handlers/activity.go`, `internal/database/reading_journey.go`, `internal/services/help_service.go`, `internal/templates/reader/interaction.html`, `internal/templates/consultant/help-requests.html`, `internal/templates/consultant/dashboard.html`, `internal/templates/consultant/readers.html`, `internal/templates/consultant/reader-inspector.html` (one-click, page-targeted AI prompt presets, including an immediate three-emoji reading-experience check-in) |
| Consultant reading journey | done | user + agent | `internal/database/reading_journey.go`, `internal/handlers/activity.go`, `internal/handlers/activity_test.go`, `internal/handlers/consultant_dashboard.go`, `internal/handlers/sse.go`, `internal/templates/reader/interaction.html`, `internal/templates/consultant/dashboard.html`, `internal/templates/consultant/reader-inspector.html` (live activity reflection with polling fallback; page-contextual, high-level Reader tool actions; unified paper palette; scrollable long history; latest-first page rail), `docs/APP_UPGRADES.md`, `docs/CONSULTANT_SIGNAL_AUTORESEARCH.md` |
| Consultant PostgreSQL activity queries | done | user + agent | `internal/handlers/reader_activity.go`, `internal/database/database.go`, `docs/EVOLUTION_CONTROL.md` (bind SQLite-style placeholders before database calls so the Render PostgreSQL deployment can load consultant activity data) |
| AI assistance UX/readability   | in-progress | user + agent | `docs/UPGRADE_ROADMAP.md`, `docs/AI_ASSISTANT_FLOW_INVENTORY.md`, `docs/READER_FIX_LIST.md`, `internal/templates/reader/interaction.html`, `internal/static/css/paper-theme.css` (paper-coordinated consultant AI suggestion card, larger linked-text starter actions, and standalone reader check-ins)                                             |
